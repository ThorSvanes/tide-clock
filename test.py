...
class chartBox:
    ...
    def buildGraph(self, data, dataMin, dataMax):
        ...
        for point in data:
            ...
            y = screenValue(point["height"], dataMin, dataMax, self.bottom, self.top)
...
def getCurrentTime(data, now = datetime.now()):
...
def getDataRange(data):
    ...
    padding = .2
    dataMin = math.floor((min(heights) - padding) * 2) / 2
    dataMax = math.ceil((max(heights) + padding) * 2) / 2
...
def drawAxisTicks(chart, tideData):
    ...
    for tick in ticks:
        y = chart.bottom - ((tick - dataMin) / (dataMax - dataMin)) * chart.height()
        drawLine((chart.left - 5, y), (chart.left, y), (100, 100, 100))
        drawLine((chart.left, y), (chart.right, y), (50, 50, 50))
        
        label = valueFont.render(f"{tick:.1f}", True, (255, 255, 255, 100))
        screen.blit(label, (chart.left - 10 - label.get_width(), y - label.get_height() // 2))

def drawChartAxis():
    pygame.draw.line(screen, (255, 0, 0), (chart1.left - 6, chart1.top), (chart1.left - 6, chart1.bottom), 3)
    pygame.draw.line(screen, (255, 0, 0), (chart1.left - 6, chart1.bottom), (chart1.right, chart1.bottom), 3)

    pygame.draw.line(screen, (180, 180, 180), (chart3.left, chart3.top), (chart3.left, chart3.bottom), 10)
    pygame.draw.line(screen, (180, 180, 180), (chart3.left, chart3.bottom), (chart3.right, chart3.bottom), 10)
...
todayLow, todayHigh = getDataRange(todayData)
weekLow, weekHigh = getDataRange(weekData)

todayGraphData = makeDetailedTideData(todayData)
weekGraphData = makeDetailedTideData(weekData)

chart1.buildGraph(todayGraphData, todayLow, todayHigh)
chart3.buildGraph(weekGraphData, weekLow, weekHigh)
...
# else:
#     time = datetime.fromisoformat("2026-07-18T04:43:00")
#     currentHours = getCurrentTime(hiloData, time)
#     print(currentHours)

currentHours = getCurrentTime(hiloData) if USE_API else getCurrentTime(hiloData, datetime.fromisoformat("2026-07-18T23:43:00"))
print(currentHours)
...
while running:
    ...
    currentHours = getCurrentTime(hiloData) if USE_API else getCurrentTime(hiloData, datetime.fromisoformat("2026-07-18T04:43:00"))
